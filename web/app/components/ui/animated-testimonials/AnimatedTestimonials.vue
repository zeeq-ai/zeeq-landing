...
const activeTestimonial = computed(() => props.testimonials[active.value])

const activeTestimonialQuote = computed(() => {
  return activeTestimonial.value?.quote.split(' ') ?? []
})

onMounted(() => {
  if (props.autoplay && props.testimonials.length > 0) {

...
Zeeq issues two kinds of user-owned credentials under **Settings > Credentials**. Both are scoped to your currently active organization, so switch organizations first if you need credentials for a different org.

Secrets are shown **only once** at creation and cannot be recovered later.

![credentials-api-tokens](/screens/credentials-api-tokens.png)

::note
Generally, most harnesses will work fine with the dynamic client registration (DCR) flow.  In some cases, you may need to use the pre-registered client flow or the API token flow (e.g. GitHub Copilot Cloud Agents).
::

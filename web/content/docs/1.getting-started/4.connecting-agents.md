...
The easiest way to connect agents is to the use the agents directly:

```bash [Terminal]
> claude "https://zeeq.ai/raw/docs/getting-started/connecting-agents.md configure this project for Zeeq for Codex and Claude Code using DCR"
```

![connecting-agents](/screens/connecting-agents.png)
...
```json [.mcp.json (DCR + dynamic skills)] {8-10}
// Per project installation in the .mcp.json file:
{
  "mcpServers": {
    "zeeq": {
...
```json [.cursor/mcp.json (Token)] {8-10}
{
  "mcpServers": {
    "zeeq": {
      "url": "https://app.zeeq.ai/mcp",
...
```json [.cursor/mcp.json (DCR + dynamic skills)] {8-10}
{
  "mcpServers": {
    "zeeq": {
      "url": "https://app.zeeq.ai/mcp",
...
3. Click **Agents**
4. Under **Repository secrets**, add a new secret with the token created from Zeeq`COPILOT_MCP_CLOUD_AGENT_ZEEQ_ACCESS_TOKEN`
...
```json [opencode.json (DCR)] {3-5,7-11}
{
...
```json [opencode.json (Token)] {11-14}
{
  "$schema": "https://opencode.ai/config.json",
  "experimental": {
    "mcp_timeout": 300000
...
```json [opencode.json (DCR + dynamic skills)] {11-13}
{
  "$schema": "https://opencode.ai/config.json",
  "experimental": {
    "mcp_timeout": 300000
...
```json [.vscode/mcp.json (Token)] {6-8,11-18}
{
  "servers": {
    "zeeq": {
      "type": "http",
...
```json [.vscode/mcp.json (DCR + dynamic skills)] {6-8}
{
  "servers": {
    "zeeq": {
      "type": "http",

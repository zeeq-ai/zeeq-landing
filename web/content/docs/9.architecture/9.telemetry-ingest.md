...
The collector runs on a Compute Engine VM behind Caddy. Caddy terminates TLS on
public `443`, keeps `80` open for automatic HTTPS challenge/redirect traffic,
and forwards OTLP/HTTP to the collector's private Docker-network listener:

```text
zeeq-otel-collector:4318
```

Do not expose collector port `4318` publicly. The public endpoint uses normal
HTTPS because OTLP/HTTP is the protocol, not a requirement to publish the
default collector port on the internet.
...
The production config keeps the same core behavior as the local filtered
collector:
...
The collector validates the token at the edge and forwards the same header to
Zeeq. Zeeq then derives user and organization scope from the authenticated
principal, not from telemetry payload attributes.
